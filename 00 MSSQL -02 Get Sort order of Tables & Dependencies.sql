...
******--------------------------------------------------------------------------------------------------------------------------------------******/
	...
	WHERE 1 = 1
		AND SCHEMA_NAME(tab.schema_id) NOT IN ('dbo', 'cdc', 'xmaster')
...
--FINAL OUTPUT::
	...
	/* THIS SHOULD BE A WORKING LOGIC **********************************************************************************************
			SELECT *, DENSE_RANK() OVER(ORDER BY SectionLevel, TableLevel, SchemaPriority) AS Hierarchy FROM #TableHierarchy
			where TargetTable IN (select tableInscope collate SQL_Latin1_General_CP1_CI_AS from ##temp)
	   Non-WORKING LOGIC ***********************************************************************************************************
			SELECT *, DENSE_RANK() OVER(ORDER BY SchemaPriority, SectionLevel, TableLevel) AS Hierarchy FROM #TableHierarchy
	*/